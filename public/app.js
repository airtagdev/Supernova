...
import { scramjetConfig, registerProxyWorker, isOwnedWorker, withTimeout } from './proxy-runtime.js?v=20260923-3';
...
function loadGames() {
  ...
  gamesPromise = fetch('/games.json?v=0.1.25-1').then(response => { if (!response.ok) throw new Error(); return response.json(); }).then(data => {
